...
- The forest's soundscape — birds, calls, rustles — has been completely ignored.
...
The system goes beyond detection into *predictive ecological awareness* — long-term acoustic baselines reveal anomalies (e.g. sudden silence in a previously active region) that point to disturbances no camera could capture.
...
2. The spectrogram is passed into *MobileNetV2*.
   - Chosen for its *depthwise separable convolutions*, which drastically cut computation and model size.
   - Ideal for fast, low-power inference on edge devices.
3. Output classes include: Gunshot, Chainsaw, Speech, Animal Vocalization, Silence Anomaly, etc.
...
2. To improve reliability, predictions are aggregated over *~10 frames* and a *voting mechanism* picks the final label.
...
| LoRa SX1278 | Long-range, low-power comms (10–15 km) |
...
- Each node communicates with its nearest neighbors and *relays* events forward.
- The mesh is *self-healing* — if one node fails, the network automatically reroutes around it.
- ~19 nodes can cover up to *~1,800 km²*.
- Independent of cellular and internet infrastructure.

The current repo ships a *2-node demo* (SENDER-NODE, RECEIVER-NODE) on the NMIT Bangalore campus block, scalable to the full 19-node mesh.
...
3. A safety filter blocks any write keywords (CREATE, MERGE, DELETE, SET, CALL db., …).
4. Cypher executes against Neo4j in READ mode.
5. Groq summarizes the rows into a concise ranger-style answer.
6. The dashboard renders the answer + the Cypher used + the raw graph rows.
...
- *Overview* — KPI tiles (critical / warning / wildlife / nodes / cam / aud), mini map, recent alerts feed, category and 24h activity charts.
- *Node Map* — full-screen Leaflet map centered on NMIT Bangalore with TX/RX markers, LoRa link, and zone overlays.
- *Live Alerts* — filterable feed (all / critical / warning / info / camera / audio / sensor).
- *Insights* — source distribution, threat breakdown, hotspot zones, AI-generated summaries.
- *AI Query* — Neo4j status, seed control, test-stream control, and the chat interface to the Groq + Neo4j agent.
...
- *Ecological anomaly detection* — sudden silence in normally active regions = early warning of disturbance.
...
1. Click *Verify* — confirms Neo4j is reachable.
2. Click *Seed India Graph* — exports zones, nodes, animals, and ~95 detections to Neo4j.
3. Ask a question.
